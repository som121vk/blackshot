...
const Store = {
    ...
    // --- UPI Settings ---
    saveUpiSettings(upiId) {
        localStorage.setItem('upi_id', upiId);
        // QR Code is now static file: images/upi-qr.jpg
    },

    getUpiSettings() {
        return {
            upiId: localStorage.getItem('upi_id') || '7985804343@fam', // Default fallback
            qrCode: 'images/upi-qr.jpg' // Static path
        };
    },
    ...
    createOrder(shippingDetails) {
        ...
        const order = {
            ...
            userEmail: currentUser ? currentUser.email : shippingDetails.email,
            userName: currentUser ? currentUser.name : shippingDetails.fullName,
            items: cart.map(item => ({
        ...
        // Store order temporarily (will be moved to pending_orders after payment)
    ...
    // --- Get Current Order ---
    getCurrentOrder() {
        return JSON.parse(localStorage.getItem('current_order') || 'null');
    },
    ...
    validateOrderForPayment(orderId) {
        ...
        if (!currentOrder) {
            return { valid: false, message: 'No active order found' };
        }
        if (currentOrder.orderId !== orderId) {
            return { valid: false, message: 'Invalid order ID' };
        }
        return { valid: true, order: currentOrder };
    },

    // --- Submit Order with Payment Proof ---
    submitOrderWithPayment(orderId, paymentProof) {
        const validation = this.validateOrderForPayment(orderId);
        if (!validation.valid) {
